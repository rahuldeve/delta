...
def chemprop(
    ...
    df, df_classification_threshold = prepare_dataset(
        dataset, use_features=train_cf.use_feats, drop_nan_features=True
    )
...
def deltaprop(
    ...
    for result_dict, predictions, split in result_iter:
        if isinstance(wandb_cf, WandbEnabled):
            model_name_suffix = wandb_cf.model_name_suffix
            model_name = "deltaprop-v6" + (f"-{model_name_suffix}" if model_name_suffix else "")
            wandb.log(  # type: ignore
                result_dict
                | asdict(model_cf)
                | asdict(train_cf)
...
def xgboost(
    ...
    df, df_classification_threshold = prepare_dataset(
        dataset, use_features=train_cf.use_feats, drop_nan_features=False
    )
    result_iter = train_and_evaluate(
        df=df,
        df_classification_threshold=df_classification_threshold,
